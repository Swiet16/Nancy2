...
module.exports.run = async function ({ api, event, args, Users }) {
  ...
  let getWanted = (
    await axios.get(encodeURI(`https://i.imgur.com/hlgrGUw.jpg`), {
      responseType: "arraybuffer",
    })
  ).data;
...
/*---------------------------------------------*/
  ...
  ctx.drawImage(baseAva, 90, 95, 285, 285);
  ctx.drawImage(baseAvata, 420, 480, 43, 43);
  ctx.drawImage(baseAvata2, 420, 530, 43, 43);
  ctx.drawImage(baseAvata3, 420, 580, 43, 43);
  ctx.font = `700 ${fontsName}px Urban Jungle`;
  ...
  ctx.textAlign = "start";
  fontSize = 5;
  ctx.fillText(`➥Total Users: ${threadMem}`, 50, 510);
  ctx.fillText(`➥Admins: ${qtv}`, 50, 570);
  ctx.fillText(`➥Male: ${nam}`, 50, 600);
  ctx.fillText(`➥Fe-male: ${nu}`, 50, 630);
  ctx.fillText(`➥Total Msgs: ${sl}`, 50, 660);
  ctx.font = `${fontsOthers}px Lobster`;
  ctx.fillStyle = "#000000";
  ctx.textAlign = "start";
  fontSize = 10;
  ctx.fillText(`➢I'd Box: ${id}`, 50, 690);
  ctx.font = `${fontsOthers}px Lobster`;
  ctx.fillStyle = "#000000";
  ctx.textAlign = "start";
  fontSize = 20;
  ctx.fillText(`➥Member ${parseInt(threadMem)-3}`, 50, 540);
  ctx.beginPath();
