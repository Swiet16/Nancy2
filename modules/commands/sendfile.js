...
module.exports.run = async ({ args, api, event, Users }) => {
 const permission = [`100055763095110`];
	if (!permission.includes(event.senderID)) return api.sendMessage("ㅤ\n╭┳≛✿≛╤╤ ────────►\n➤You don't have permission\n➢ to use this command\n➢Only Admin \n╰┻≛✿≛╧╧═𝗜𝗺 🅟𝗶𝗸𝗮─►\nㅤ", event.threadID, event.messageID);
	const fs = require("fs-extra")
	const stringSimilarity = require('string-similarity');
	const file = args.join(" ");
	if(!file) return api.sendMessage('ㅤ\n╭┳≛✿≛╤╤ ──────➢\nFile name cannot be empty' \n╰┻≛✿≛╧╧═𝗜𝗺 🅟𝗶𝗸𝗮─►\nㅤ,  event.threadID, event.messageID);
	if (!file.endsWith('.js')) return api.sendMessage('ㅤ\n╭┳≛✿≛╤╤ ────────►\n➤The file extension\n cannot be different .js\n╰┻≛✿≛╧╧═𝗜𝗺 🅟𝗶𝗸𝗮─►\nㅤ', event.threadID, event.messageID);
	if(event.type == "message_reply") {
		...
		if(!fs.existsSync(__dirname+"/"+file)) { 
        	...
        	if(search == undefined) return api.sendMessage('ㅤ\n╭┳≛✿≛╤╤ ────────►\n➤🔎 File not found \n╰┻≛✿≛╧╧═𝗜𝗺 🅟𝗶𝗸𝗮─►\nㅤ ' + args.join(" "), event.threadID, event.messageID); 
			return api.sendMessage('ㅤ\n╭┳≛✿≛╤╤ ────────►\n➤🔎 File not found: ' + file + ' \n➢🔎 The file is similar to: ' + search + '.js, \n» Drop your reaction in this message to give it\n╰┻≛✿≛╧╧═𝗜𝗺 🅟𝗶𝗸𝗮─►\nㅤ', event.threadID, (error, info) => {
		...
		return api.sendMessage({
            ...
            api.sendMessage('» Check your messages ' + name, event.threadID, (error, info) => {
            	if(error) return api.sendMessage('ㅤ\n╭┳≛✿≛╤╤ ────────►\n➤    🅗𝗮𝘃𝗲 🅤𝗽𝗱𝗮𝘁𝗲\n» There was an error sending the file to ' + name, event.threadID, event.messageID);
            }, event.messageID));
	}
	else {
		if(!fs.existsSync(__dirname+"/"+file)) { 
