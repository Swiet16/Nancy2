...
module.exports.handleReply = async function ({ api, event, handleReply }) {
	...
	if (isNaN(num)) msg = "😜😜😜Why deleted";
		...
		if (handleReply.type == "cmd_info") {
			...
			msg += ` ➢  ${command_config.commandCategory.toUpperCase()}   𖤂   \n`;
...
module.exports.run = async function({ api, event, args }) {
	...
	if (args[0] && ["all", "-a"].includes(args[0].trim())) {
		...
		index_start = (page_num_input) ? (page_num_input * 2222222222) - 2222222222 : 0;
			...
			all_commands.forEach(e => {
				msg += `\n➢${index_start+=1}𖤹 ➡︎${e}: ${commands.get(e).config.description}`;
			})
			msg += `                ➥𝗣𝗮𝗴𝗲 : ${page_num_input || 1}/${page_num_total}`;
			msg += ``
			msg += "\n╰────────╯\nEdited By AZIZ😜   \n╰────────╯";
		}
		var msgg = {body: msg, attachment: imgP}
	...
	if (check) {
		...
		group.forEach(commandGroup => msg += `\nㅤ➣${index_start+=1}𖤂 ➡︎ ${commandGroup.group.toUpperCase()} `);
		msg += `╰♥︎───────────╯  💖\n                ➥𝗣𝗮𝗴𝗲 : ${page_num_input || 1}/${page_num_total}`;
    msg += `\n➡︎Hey I'm Pika😜`
		msg += `\n*★O͠W͟͟N͜͡ETR͟͟   ཫ 𓆩♥︎🅐z͜͡izིྀིྀིྀིྀིྀིྀིྀི𓆪
 \nㅤ`;
	}
	var msgg = {body: msg, attachment: imgP}
