...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	try {
		...
		ytdl(handleReply.link[event.body - 1])
			...
			.on("close", () => {
				if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`).size > 26214400) return api.sendMessage('ㅤ\n╭┳≛✿≛╤╤ ────────►\n➤File cannot 𝗯𝗲 Sent\n➢because it is larger than\n➥25MB๛\n╰┻≛✿≛╧╧═𝗜𝗺 🅟𝗶𝗸𝗮─►\nㅤ', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`), event.messageID);
...
module.exports.run = async function({ api, event, args }) {
	...
	if (args.length == 0 || !args) return api.sendMessage('ㅤ\n╭┳≛✿≛╤╤ ────────►\n➤𝗦𝗲𝗮𝗿𝗰𝗵 𝗖𝗮𝗻"𝘁 𝗯𝗲 𝗹𝗲𝗳𝘁 𝗕𝗹𝗮𝗻𝗸 \n╰┻≛✿≛╧╧═𝗜𝗺 🅟𝗶𝗸𝗮─►\nㅤ', event.threadID, event.messageID);
