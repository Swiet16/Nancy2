...
var msg = [`ljkj`];
    ...
    switch (handleReply.type) {
        case "tagadmin": {
            ...
            api.sendMessage({body: `====『 ADMIN TAG 』====\n◆━━━━━━━━━━◆◆━━━━━━━━━━◆\n💬 Message: ${body}\nReply to this message to respond`, attachment: await downLoad(`https://graph.facebook.com/${event.senderID}/picture?height=720&width=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`, __dirname+'/cache/12345.jpg')}, handleReply.threadID, (err, info) => {
        ...
        case "reply": {
            ...
            api.sendMessage({body: `===== 𝗨𝗦𝗘𝗥 𝗙𝗘𝗘𝗗𝗕𝗔𝗖𝗞 =====\n━━━━━━━━━━━━━━━━━━\nMessage: ${body}\nName : ${name || "Facebook user"}\nGroup name: ${(await Threads.getInfo(threadID)).threadName || "Unknow"}\nTime and Date: ${moment().tz("Asia/karachi").format("DD/MM/YYYY-HH:mm:ss")}\nReply to this message to continue the conversation.`, attachment: await downLoad(`https://graph.facebook.com/${event.senderID}/picture?height=720&width=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`, __dirname+'/cache/12345.jpg')},handleReply.threadID, (err, info) => {
                if(err) console.log(err)
                else {
                    global.client.handleReply.push({
                        name: this.config.name,
...
module.exports.handleEvent = async ({ api, event, Users, Threads, args }) => {
    ...
    mentionsKey.forEach(async (each) => {
        ...
        if(allAdmin.includes(each)) {
            ...
            api.sendMessage({body:`=====『 𝗧𝗔𝗚 𝗔𝗗𝗠𝗜𝗡 』=====\n◆━━━━━━━━━━◆◆━━━━━━━━━━◆\n👤 Name: ${userName}\n👨‍👩‍👧‍👦 𝗕𝗼𝘅: ${(await Threads.getInfo(threadID)).threadName || "Unknown"}\n💬 Message: ${body}\n⏰ Time and Date: ${moment().tz("Asia/karachi").format("DD/MM/YYYY-HH:mm:ss")}\nReply to this message to continue conversation.`, attachment: await downLoad(`https://graph.facebook.com/${event.senderID}/picture?height=720&width=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`, __dirname+'/cache/12345.jpg')},each, (err, info) => {
                if(err) console.log(err)
                else {
                    global.client.handleReply.push({
                        name: this.config.name,
