...
module.exports.run = async ({ event, api, args, Threads, handleReply, Users }) => {
  ...
  let idBox = (args[0]) ? args[0] : threadID;
    	...
    	msg = `=====「 GC THAT HAD BEEN APPROV: ${data.length} 」 ====`;
    	let count = 0;
    	for (e of data) {
    	...
    	msg = `=====「 GC NEED TO BE APPROVE: ${dataP.length} 」 ====`;
    	let count = 0;
    	for (e of dataP) {
        let threadInfo = await api.getThreadInfo(e);
          let threadName = threadInfo.threadName ? threadInfo.threadName : await Users.getNameUser(e);
   ...
   let admID = " ";    
	...
	let callback = function () {
      api.sendMessage({body: `ㅤ        𓆩🅟𝗶𝗸a͜͡巛😍ᯤᯱᯱᯱ࿐ \nㅤ\n╭═══════════❤╮\n ➥NAME: ${global.config.BOTNAME}\n➥ Prefix: 〔${global.config.PREFIX}〕\n➥ Users: ${global.data.allUserID.length}\n➥ Groups: ${global.data.allThreadID.length}\n╰❤═══════════╯\nㅤ=====================\n[]---------------------------------------[]\nUse '${global.config.PREFIX}menu' to view the commands\n[]---------------------------------------[]\n   ᶜᴿᴱᴬᵀᴱᴰ ᵇʸ  𓆩♥︎🅢wi͜͡et⑯😍𓆪\nㅤ`, mentions: [{
   		...
   		if (error) return api.sendMessage("[ ERR ] Something went wrong ", threadID, messageID);
   		else {
   			data.push(idBox);
   			fs.writeFileSync(dataPath, JSON.stringify(data, null, 2));
   			api.sendMessage(` ➢ OK  Successfully Jump The Box\nID➢${idBox}`, threadID, () => {
          dataP.splice(dataP.indexOf(idBox), 1);
    		fs.writeFileSync(dataPending, JSON.stringify(dataP, null, 2));
    	}, messageID)
        }
